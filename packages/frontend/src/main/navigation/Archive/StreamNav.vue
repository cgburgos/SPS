              ...
              >
                <template #activator>
                  <v-list-item style="overflow: visible">
                    ...
                    <v-list-item-content style="position: relative; left: -8px">
                      <v-list-item-title>{{ item.ori }}</v-list-item-title>
                      <v-list-item-subtitle class="caption">
                        {{ item.children.length }} branches
                      </v-list-item-subtitle>
                    </v-list-item-content>
    ...
    groupedBranches() {
      ...
      for (const b of branches) {
        ...
        if (parts.length === 1) {
          ...
          //CREATE HERE DIFFERENT LISTS IN CASE
          let existing = items.find(
            (i) => i.name === parts[0] && i.type === 'specialtyGroup'
          )
          // if (!existing) {
          //   existing = {
          //     name: parts[0],
          ...
          if (b.commits.items[0] !== undefined) {
            lastCommit = `${b.commits.items[0].message}`
          }
