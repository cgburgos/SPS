<template>
  <v-hover v-slot="{ hover }">
    <v-card
      ...
      :style="`${
        hover
          ? 'border: 1px solid #d2dad3; max-width: 250px; background-color: #f1f2f0;'
          : 'border: 1px solid #d2dad3; max-width: 250px'
      }`"
    >
      <router-link
        ...
        <div class="mx-1 mb-2">
          <v-card-text class="caption d-flex pb-2 pt-0">
            ...
            <!-- <div class="text-right flex-grow-1">
              <span>at {{ new Date(commit.createdAt).toLocaleTimeString() }}</span>
            </div> -->
          </v-card-text>
        </div>
